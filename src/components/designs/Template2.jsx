...
const Template1 = () => {
  ...
  const handleSkillsChange = (index, event) => {
    ...
    if (name === "percentage") {
      ...
      updatedSkills[index][name] = isNaN(numericValue)
        ? ""
  ...
  const generatePDF = async () => {
    if (isEdit) {
      return toast.info("Please save file first!");
    }
    const element = resumeRef.current;
    if (!element) {
    ...
    htmlToImage
      ...
      .then(function (dataUrl) {
        ...
        const pdf = new jsPDF("p", "mm", [a4height, a4width]);
        ...
        pdf.save(user.displayName + Date.now());
      })
      .catch((error) => {
  ...
  const generateImage = async () => {
    ...
    htmlToImage
      ...
      .then(function (dataUrl) {
        ...
        link.download = user.displayName + Date.now() + ".jpeg";
        link.href = dataUrl;
        link.click();
      })
      .catch((error) => {
  ...
  const generatePng = async () => {
    ...
    htmlToImage
      ...
      .then(function (dataUrl) {
        const link = document.createElement("a");
        link.download = user.displayName + Date.now() + ".png";
        link.href = dataUrl;
        link.click();
      })
      .catch((error) => {
  ...
  const generateSvg = async () => {
    ...
    htmlToImage
      ...
      .then(function (dataUrl) {
        const link = document.createElement("a");
        link.download = user.displayName + Date.now() + ".svg";
        link.href = dataUrl;
        link.click();
      })
      .catch((error) => {
  ...
  if (resumeIsLoading) return <MainSpinner />;
